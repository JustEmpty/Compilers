...
void fir::type_checker::do_with_node(fir::with_node *const node, int lvl) {
    node->vector()->accept(this, lvl);
    if (!node->vector()->is_typed(cdk::TYPE_POINTER)) throw std::string("expected pointer for vector");

    node->low()->accept(this, lvl);
    if (!node->low()->is_typed(cdk::TYPE_INT)) throw std::string("expected integer for low");

    node->high()->accept(this, lvl);
    if (!node->high()->is_typed(cdk::TYPE_INT)) throw std::string("expected integer for high");

    const std::string &id = node->function();
    auto symbol = _symtab.find(id);
    if (!symbol) throw std::string("symbol '" + id + "' is undeclared.");
    if (!symbol->isFunction()) throw std::string("symbol '" + id + "' is not a function.");
    if (symbol->number_of_arguments() != 1)  throw std::string("number of arguments in function '" + id  + "' must be 1.");
    if (!symbol->argument_is_typed(0, cdk::reference_type::cast(node->vector()->type())->referenced()->name()))
        throw std::string("'" + id  + "' argument type must match type of pointer");
